...
var initData = function() {
    ...
    $('.mtr-name').text(gzmtr.name);
    var lines = gzmtr.getLines();
    //初始化路线
...
var initEvent = function() {
    var startLine = $('.start .line');
    var endLine = $('.end .line');
    var startTerminal = $('#Start_2');
    var endTerminal = $('#End_2');
    var inst1 = new mdui.Select('#Start_2',{position: 'bottom'});
    var inst2 = new mdui.Select('#End_2',{position: 'bottom'}); 
    //改变起点和终点的路线时，重置路线途径的地铁站点
    startLine.on('change', function() {
        var lineIndex = $(this).val();
        var line = gzmtr.getLine(lineIndex);
        //var inst1 = new mdui.mutation('#Start_2');
        console.log(line);
        startTerminal.empty();
        //inst1.handleUpdate();
    ...
    endLine.on('change', function() {
        var lineIndex = $(this).val();
        var line = gzmtr.getLine(lineIndex);
        //var inst2 = new mdui.mutation('#End_2');
        console.log(line);
        endTerminal.empty();
        //inst2.handleUpdate();
        line.getTerminals().forEach(function(terminal, i) {
  ...
  gzmtr.reset();
  var route = gzmtr.search(gzmtr.getLine(startLine).getTerminal(startTerminal), gzmtr.getLine(endLine).getTerminal(endTerminal));
  $('.desc').html(route.time + '分钟<br/>' + route.desc);
...
gzmtr.addLines([line1, line2, line3, line3plus, line4, line5, line6]);
initData();
initEvent();

// 已通过调用svg-zoom-pan方法实现自适应功能
//window.onresize = function(){
    ...
    //_autoZoom();
    // function _autoZoom(){
    //      var svg = document.getElementById("mainViewContainer");
    //     if (svg) {
    ...
    //             svg.setAttribute("width", setWidth-340);
    //             svg.setAttribute("height", setHeight*0.9);
    //         }
    //     }
    // }
    ...
    //     var ViewContainer = document.getElementById("mainViewContainer");
    //     var html = document.getElementsByTagName("body");

    //     ViewContainer.setAttribute("width",window.innerWidth-340);
    //     ViewContainer.setAttribute("height",window.innerHeight-20);
    // }
